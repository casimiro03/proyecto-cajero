"""
Módulo de operaciones: implementa las operaciones bancarias del cajero.
"""
...
def extraer_dinero(dni):
    ...
    print(f"  Límite por operación: {utilidades.formatear_monto(datos.LIMITE_EXTRACCION_DIARIA)}")
    print("-" * 40)
    ...
    # Validaciones
    if monto > datos.LIMITE_EXTRACCION_DIARIA:
        print(f"  ✗ El monto supera el límite de extracción ({utilidades.formatear_monto(datos.LIMITE_EXTRACCION_DIARIA)}).")
        return
...
def ver_historial(dni):
    ...
    if not historial:
        print("  No hay operaciones registradas en esta sesión.")
        return
